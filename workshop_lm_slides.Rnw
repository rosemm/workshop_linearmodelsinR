...
\begin{itemize}
   \item<1-> Focus on the R code rather than the stats --- if you'd like to learn about the stats behind the general linear model more deeply, I can recommend several excellent classes and texts.
    ...
    \item<3-> We'll be using \verb+haven+ to read in an SPSS data file and \verb+ggplot2+ for plotting
  \end{itemize}
...
\keyidea{Models in lm() are specified as a "formula", like this: outcome ~ predictors}
...
\keyidea{In the lm() function, you refer to variables in the data by their bare names, without using $.}
