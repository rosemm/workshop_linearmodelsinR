...
\title[R Workshop]{Intro to linear models in R}
...
\begin{itemize}
   \item<1-> Linear models are some of the most common (and useful!) tools in data analysis.
    \item<2-> R is a fantastic choice for reproducible analyses.
...
If you don't already have \verb+ggplot2+, and \verb+haven+ installed, do that now:
<<>>=
...
\seealso{There are many packages for making R output into nice tables, including the knitr::kable() that comes standard in R Studio, and \href{https://bookdown.org/yihui/rmarkdown-cookbook/table-other.html}{many others}. I find pander yields the most professional results in Word output.}
...
Note: I recommend car::Anova() over stats::aov() because it allows you to use type 3 sums of squares, which is probably what you want (it's what SPSS and SAS use).
...
% -------------------------------------------------------------
% -------------------------------------------------------------
\begin{frame}[fragile]{Key ideas from this section: More models}
