...
\begin{itemize}
  ...
  \item<2-> The computer uses brute force to find a solution, iterating over tons of combinations of parameter estmiates and seeing which gives the bets model fit
...
<<>>=
osf.lgt <- osf %>% 
  select(date, statement.included) %>% 
  na.omit()
...
Let's run the model.
<< model >>=
logit.model <- glm(statement.included ~ date, data=osf.lgt,
                   family=binomial(link="logit"),
                   na.action=na.exclude)
@
...
It's typical to test a series of glms, beginnning with a null model (no predictors).
<< >>=
...
<<>>=
anova(logit.model0, logit.model, test="Chisq") 
@
...
For logistic regresion, it's nice to have a classification table showing your model's predicted outcomes compared to the actual outcome.
\pause
<< predict >>=
osf.lgt$pred <- predict(logit.model, 
                                osf.lgt, 
                                type="response") 
...
Convert the new classifcation variables to factors, for cleaner output in the crosstabs.
<< clas_factor >>=
...
xtabs(~ statement.included + clas, data=osf.lgt)
@
