...
\begin{itemize}
   \item<1-> Focus on the R code rather than the stats --- if you'd like to learn about the stats behind MLM more deeply, I can recommend several excellent classes and texts.
    ...
    \begin{itemize}
	    \item<5-> interleaved training and progressive alignment
	    \item<6-> realistic practice problems from the two perspectives you're most likely to encounter in real life
	    \item<7-> consistent color-coding of different types of information 
	    \end{itemize}
  \end{itemize}
...
\begin{itemize}
    ...
    \item<3-> Random effects (when you want to generalize from a sample to a population)
  \end{itemize}
...
\bigskip
\pause
But having the tools necessary to run a multilevel model is definitely a good idea!

\end{frame}
% -------------------------------------------------------------
% -------------------------------------------------------------
\begin{frame}[fragile]{What do you need to think about when chosing MLM software?}

\begin{itemize}
    \item<1-> logistics (collaborators' preferences, price, availability, etc.)
    \item<2-> 
  \end{itemize}
...
\begin{itemize}
  ...
  \item<3->You need to load each package you want to use again in each R session. 
\end{itemize}
...
\begin{itemize}
  ...
  \item<2-> You have several different sources of error variance (and covariances among them!)
  \item<3-> There is no longer an analytic solution --- the computer has to brute force a solution by trying millions of combinations of parameter estimates and seeing what yields the best fit.
\end{itemize}
...
\pause
\bigskip
Find the file on your computer, get its location, and add that file path to your read_sav command:
<< >>=
...
\end{itemize}
\pause
Centering only makes sense for continuous variables. Be careful to never try to center a categorical predictor. 
...
\subsection{Example 1: Nested data}
% -------------------------------------------------------------
\begin{frame}[fragile]{Overview}
\tableofcontents[ 
    currentsubsection, 
...
\subsection{Example 2: Longitudinal data}
% -------------------------------------------------------------
\begin{frame}[fragile]{Overview}
\tableofcontents[ 
    currentsubsection, 
...
\subsection{Example 3: Random effect}
% -------------------------------------------------------------
\begin{frame}[fragile]{Overview}
\tableofcontents[ 
    currentsubsection, 
