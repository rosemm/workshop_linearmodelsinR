...
<<>>=
model1 <- lmer(STSE1 ~ intervention * STSE0 + 
                 (1|schoolid), 
              data = atlas,
              na.action = na.exclude)
@
...
\pause
This estimates the covariance between slope for STSE0 and the intercept:
<<>>=
model2 <- lmer(STSE1 ~ intervention * STSE0 + 
                 (1 + STSE0|schoolid), 
              data = atlas,
              na.action = na.exclude)
@
...
<<>>=
model2.nocov <- lmer(STSE1 ~ intervention * STSE0 + 
                 (1|schoolid) + (0 + STSE0|schoolid), 
              data = atlas,
              na.action = na.exclude)
@
...
% -------------------------------------------------------------
% -------------------------------------------------------------
\begin{frame}[fragile]{Additional resources about repeated measures}

\begin{itemize}
  \item This excellent blog post describes when a repeated measures ANOVA won't work and what you can do instead (hint, in most cases, a mixed effects model is a great solution) \url{http://www.theanalysisfactor.com/when-repeated-measures-anova-not-work-for-repeated-measures-data/}
\end{itemize}
